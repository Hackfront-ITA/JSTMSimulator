...
db.each('SELECT id, username, code, timestamp FROM userdata ORDER BY timestamp DESC', function(err, curruserdata){
	...
	Object.keys(serverConfig.users).forEach(function(currusername){
		...
		var contestTableHtml = [
			'<div id="results">',
			'  <h1 align="center">Verifica della gara del ' + lastsavedate.getDate() + '/' + (lastsavedate.getMonth() + 1) + '/' + lastsavedate.getFullYear() + '</h1>',
			'  <p>Per provare usare il <a href="#simulator">simulatore</a> pre-caricato con i problemi della squadra \'<i>' + sanitizer.escape(String(currusername)) + '</i>\'.</p>',
			'  <h2>Sommario (' + points + ' punt' + (points === 1 ? 'o' : 'i') + ', ultimo salvataggio ' + lastsavetime + ')</h2>',
			'  <table border="1">',
			'    <tr>',
			'      <th>Problem</th>',
			'      <th>Points</th>',
		...
		Object.keys(contestProblems).forEach(function(currproblemid){
			...
			var curruserproblem = currproblem.userdata[currusername];
			contestTableHtml += [
				'<h2><a name="' + sanitizer.escape(String(currproblemid)) + '">' + sanitizer.escape(String(currproblem.name)) + '</a> (' + currproblem.points + ' punt' + (currproblem.points === 1 ? 'o' : 'i') + ')</h2>',
				'<table border="1">',
				'  <tr>',
				'    <th>Outcome</th>',
				'    <th>Input</th>',
			...
			if (curruserproblem.error) {
				...
				curruserproblem.testcases.forEach(function(currtestcase){
					contestTableHtml += [
						'<tr>',
						'  <td>' + sanitizer.escape(String(currtestcase.outcome)) + '</td>',
						'  <td class="codecell">'+sanitizer.escape(String(currtestcase.initialtape)) + '</td>',
						'  <td class="codecell">'+sanitizer.escape(String(currtestcase.finaltape)) + '</td>',
						'  <td class="codecell">'+sanitizer.escape(String(currtestcase.expectedtape)) + '</td>',
						'  <td>' + currtestcase.steps + '</td>',
						'  <td class="' + (currtestcase.success ? 'green' : 'red') + '">' + (currtestcase.success ? 'OK' : 'FAIL') + '</td>',
						'</tr>'
					].join('\n');
				});
			}
		...
		fs.writeFileSync(path.join(resultsDirName, currusername, 'index.html'), [
			'<!doctype html>',
			...
			'  <title>Turing Machine Competition Results</title>',
			htmlheadHtml,
			'  <style>',
			uglifycss(String(cssStyle)),
			'  </style>',
			iecsshacksHtml,
			'</head>',
			'<body class="loadmode displayresults">',
			contestTableHtml,
			turingMachineHtml,
			'  <script>',
			uglifyjs.minify('(function(){' + turingMachineJS + i18nJS + handleHTMLPageJS + 'var preloadedProblems = ' + JSON.stringify(preloadedProblems) + ';' + handlePreloadedProblemsJS + '})();', {fromString: true}).code,
			'  </script>',
			'</body>',
			'</html>'
	...
	var contestTableHtml = [
		'<div id="results">',
		'<h1 align="center">Classifica della gara del ' + lastsavedate.getDate() + '/' + (lastsavedate.getMonth() + 1) + '/' + lastsavedate.getFullYear() + '</h1>',
		'<h2>Ultimo salvataggio globale: ' + lastsavetime + '</h2>',
		'<table border="1">',
		'<tr>',
		'<th>Posizione</th>',
		'<th>Nome utente</th>',
	...
	fs.writeFileSync(path.join(resultsDirName, 'index.html'), [
		'<!doctype html>',
		'<!-- saved from url=(0014)about:internet -->',
		'<!--',
		licenseText,
