...
var tickSpeed = null;
var drawSpeed = null;
var speedToTimeoutMapping = [Infinity, 1500, 1350, 1200, 1050, 900, 750, 600, 450, 300, 150, 75, 0, -1];
...
setTextContent(statusDiv, currlang.STATUS_READY);
startBtn.value = currlang.START_BUTTON;
stopBtn.value = currlang.STOP_BUTTON;
outputBtn.value = currlang.OUTPUT_BUTTON;
...
var removeEvent = function(elm, evt, func){
	if (elm.removeEventListener) {
		elm.removeEventListener(evt, func, false);
	} else if (elm.detachEvent) {
		elm.detachEvent('on' + (evt === 'change' ? 'propertychange' : evt), func);
	}
};
...
var drawtape = function(){
	...
	if (currclass === 0 && turingMachineInstance.stopped) {
		...
		return;
	} else if (currclass === 0 && tickSpeed === Infinity) {
		return;
	}
	...
	turingMachineInstance = new TuringMachine({
		...
		onprestart: function(){
			if (tickSpeed !== Infinity) {
				...
				removeEvent(startBtn, 'click', start);
				removeEvent(startBtn, 'click', step);
				addEvent(startBtn, 'click', step);
			}
			codeTextarea.readOnly = inputBox.disabled = progsSelect.disabled = loadBtn.disabled = saveBtn.disabled = true;
			beforeUnloadWarningEnabled = true;
			currclass = null;
			if (drawTimeout !== -1)  clearTimeout(drawTimeout);
			if (tickTimeout !== -1)  clearTimeout(tickTimeout);
		},
		onstart: function(){
			setstatus('');
...
preventSelectTo = null;
selectPreventerEvent = null;
selectPreventerFunc = null;
...
addEvent(outputBtn, 'click', function(){
	if (turingMachineInstance) {
		alert(currlang.TAPE_TEXT_LABEL + ' ' + JSON.stringify(turingMachineInstance.tapetext.trim()));
	}
});
...
addEvent(speedSelect, 'change', function(e, v){
	...
	if (tickSpeed === Infinity) {
		removeEvent(startBtn, 'click', start);
		removeEvent(startBtn, 'click', step);
		if (turingMachineInstance && !turingMachineInstance.stopped) {
			addEvent(startBtn, 'click', step);
			startBtn.disabled = false;
